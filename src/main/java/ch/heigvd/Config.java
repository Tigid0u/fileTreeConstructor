...
public class Config {
  ...
  /**
   * Construct a construct object with default attributes values: <br>
   * <br>
   * {<br>
   * encoding = UTF-8,<br>
   ...
   * @param notesFileFormat file format of the course notes. Supported file formats {.txt, .md}.
   *     Default: .md (markdown)
   * @param subdirs subdirectories to create when creating a new class file tree. Default: {"class
   *     material", "course notes"}
   */
  public Config(
      String encoding,
      boolean includeDateInFilename,
      boolean includeClassNameInFilename,
      String notesFileFormat,
      String[] subdirs) {
    try {
      // Check if the given encoding is valid and converts it from a String to an actual charset
      // object
    ...
    this.subdirs =
        (subdirs != null) ? subdirs.clone() : new String[] {"class material", "course notes"};
   ...
   * @throws IOException if the file doesn't exist of failed to open. Or if the JSON doesn't
   *     correspond to the Config object structure
   */
  ...
  public String toString() {
    return "Encoding="
        ...
        + this.includeDateInFilename
        + ", includeClassNameInFilename="
        + this.includeClassNameInFilename
        + ", subdirs=["
        + String.join(",", this.subdirs)
        + "], noteFileFormat="
        + this.notesFileFormat;
  }
}
